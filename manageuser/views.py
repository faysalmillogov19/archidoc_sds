...
def index(request):
    ...
    if request.POST:
        ...
        if user is not None:
            ...
            return redirect(
                "/"
            )  # serializers.serialize('user', user) #JsonResponse(data)
        else:
            message = (
                "Nom utilisateur ou mot de passe incorrecte. Vérifier vos informations."
            )
            return render(
                request,
                "registration/login.html",
                {"email": email, "password": password, "message": message},
            )
    else:
