...
def save(request):
    ...
    doc.type_document = Type_document.objects.get(
        id=int(request.POST.get("type_document"))
    )
    doc.annee_academique = Annee_academique.objects.get(
        id=int(request.POST.get("annee_academique"))
    )
...
def found(request):
    if request.method == "POST":
        data = Etudiant.objects.filter(
            annee_academique=int(request.POST.get("annee_academique"))
        )
